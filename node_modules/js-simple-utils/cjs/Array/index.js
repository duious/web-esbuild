...
const initArray = (obj, onlyTrueValue = false) => {
    if (isArray(obj)) {
        return onlyTrueValue ? obj.filter((item) => Object_index.isTrueValue(item)) : obj;
    }
    return [];
};
/**
...
const fill = (list, length, value = '', onlyTrueValue = false) => {
    ...
    return (Array.from(Object.assign(Object.assign({}, list), { length }))
        ...
        .filter(item => (onlyTrueValue ? (Object_index.isTrueValue(item) ? item : false) : true))
        .map((item) => {
        // eslint-disable-next-line no-nested-ternary
        return onlyTrueValue ? (Object_index.isTrueValue(item) ? item : value) : value;
    }));
};
/**
 * 数组遍历回调函数
...
var index = { isArray, initArray, fill, forEach };
