...
var Date_local = require('./local.js');
...
const parseDate = (date, utc) => {
    ...
    if (typeof date === 'string' && !/Z$/i.test(date)) {
        const d = date.match(RegExp_index.DATE_STRING);
        if (d) {
...
const format = (date, template = FORMAT_DEFAULT, loc = 'zh') => {
    ...
    const { weekdays, weekdaysShort, months, monthsShort, meridiem, weekdaysMin } = Date_local[loc];
    ...
    return template.replace(RegExp_index.DATE_STRING_FORMAT, (match, $1) => $1 || matches[match] || zoneStr.replace(':', ''));
};
