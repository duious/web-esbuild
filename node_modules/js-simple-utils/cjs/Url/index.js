 ...
 */
const URL_REGEXP = RegExp_index.URL;
const formatVal = (val) => {
    if (RegExp_index.isNumber(String(val))) {
        return Number(val);
    }
...
const getUrlParams = (url, name) => {
    ...
    if (URLSearchParams !== undefined) {
        ...
        if (Object_index.isTrueValue(name)) {
            const arr = Array_index.initArray(params.getAll(name)).map(item => formatVal(item));
            return arr.length === 1 ? arr[0] : arr;
        }
        res = {};
        params.forEach((val, key) => {
            if (Object.prototype.hasOwnProperty.call(res, key)) {
                if (Array_index.isArray(res[key])) {
...
const parseSearchQuery = (querySearchStr, key) => {
    ...
    if (Object_index.isTrueValue(key)) {
        Array_index.forEach(arr, (item) => {
            if (Object.prototype.hasOwnProperty.call(item, key)) {
                if (Object_index.isTrueValue(res)) {
...
function setUrlParams(url, ...rest) {
    ...
    if (rest.length === 1) {
        params = [];
        Array_index.forEach(Object.entries(rest[0]), ([key, value]) => {
            if (Array_index.isArray(value)) {
                Array_index.forEach(value, (item) => {
    ...
    if (rest.length === 2) {
        if (Array_index.isArray(rest[1])) {
            params = [];
            Array_index.forEach(rest[1], (item) => {
    ...
    return Object_index.isTrueValue(search)
 ...
 */
const isUrl = RegExp_index.isUrl;
...
exports.stringifySearchQuery = stringifySearchQuery;
